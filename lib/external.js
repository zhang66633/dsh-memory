...
/**
 * Validate a parsed unit document against one descriptor; throws with a
 * specific message on any violation.
 * @param document - parsed JSON value.
 * @param descriptor - `{name, version}` the unit must declare.
 * @returns the same document.
 */
export function validateUnit(document, descriptor) {
  if (typeof document !== 'object' || document === null || Array.isArray(document)) {
    throw new Error('unit file must contain a JSON object')
  }
  const unit = document.unit
  if (typeof unit !== 'object' || unit === null
    || unit.name !== descriptor.name
    || unit.version !== descriptor.version) {
    throw new Error(`unit file must declare unit ${descriptor.name} version ${descriptor.version}`)
  }
  const tables = document.tables
  if (tables !== undefined
    && (typeof tables !== 'object' || tables === null || Array.isArray(tables))) {
    throw new Error('unit file tables must be an object')
  }
  const entries = tables?.[ENTRIES_TABLE]
  if (entries !== undefined
    && (typeof entries !== 'object' || entries === null || Array.isArray(entries))) {
    throw new Error(`unit file table '${ENTRIES_TABLE}' must be an object`)
  }
  return document
}

/**
 * Validate a parsed document against the documented memory format; throws
 * with a specific message on any violation.
 * @param document - parsed JSON value.
 * @returns the same document.
 */
export function validateDocument(document) {
  return validateUnit(document, { name: MEMORY_DOMAIN, version: UNIT_VERSION })
}

/**
 * Load one unit file as a generic document; a missing file yields the empty
 * unit for the descriptor (`global: null`, every table empty). Returns the
 * parsed document plus the file mtime for optimistic concurrency.
 * @param path - absolute unit file path.
 * @param descriptor - `{name, version}` identity stamped on the medium.
 */
export async function loadUnitFile(path, descriptor) {
  let text
  let mtimeMs = 0
  try {
    ...
    text = JSON.stringify({
      unit: { name: descriptor.name, version: descriptor.version },
      global: null,
      tables: {},
    })
  }
  ...
  try {
    ...
    throw new Error(`unit file is not valid JSON: ${error?.message ?? error}`)
  }
  validateUnit(document, descriptor)
...
export async function loadMemoryFile(path) {
  const loaded = await loadUnitFile(path, { name: MEMORY_DOMAIN, version: UNIT_VERSION })
  loaded.document.tables ??= {}
  loaded.document.tables[ENTRIES_TABLE] ??= {}
  if (loaded.document.global === null || loaded.document.global === undefined) {
    loaded.document.global = { last_extracted: {} }
  }
  return loaded
}
