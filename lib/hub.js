...
export function apply(ctx, config = {}) {
  ...
  const wrapUnit = (unit) => ({
    ...
    table: (tableName) => ({
      ...
      remove: (key) => unit.deleteRecord(tableName, key),
    }),
    close: () => unit.close(),
  })
  ...
  const service = {
    ...
    /** Permanently delete a memory. Irreversible — no tombstone, no restore. */
    async purge(id) {
      const t = await table()
      const record = t.get(id)
      if (record === undefined) return { ok: false, error: `memory '${id}' not found` }
    ...
    /**
     * Entity co-occurrence data for the panel: `cards` (the full entity
     * list, most-connected first) and `graph` (the stable core — only
     * relationships observed in at least `minEdgeWeight` memories, so the
     * canvas draws structure instead of one-off prose coincidences).
     * @param options - caps keeping both views bounded.
     */
    async graphData({ maxCards = 40, maxNodes = 30, maxEdges = 40, minEdgeWeight = 2 } = {}) {
      const entries = (await allEntries()).filter((r) => !r.tombstone && !r.archived)
      const { nodes, edges } = buildGraph(entries)
      const allEdges = [...edges.values()]
      // Cards: the full entity list, most-connected first, with their top
      // relations embedded so the card view needs no client-side join.
      const cards = [...nodes.values()]
        .sort((a, b) => b.degree - a.degree || b.count - a.count || b.lastSeen - a.lastSeen)
        .slice(0, maxCards)
        .map((node) => ({
          ...node,
          relations: allEdges
            .filter((edge) => edge.source === node.text || edge.target === node.text)
            .map((edge) => ({ other: edge.source === node.text ? edge.target : edge.source, weight: edge.weight }))
            .sort((a, b) => b.weight - a.weight)
            .slice(0, 8),
        }))
      // Graph core: only stable relationships — entities co-occurring in at
      // least `minEdgeWeight` memories. One-off prose coincidences stay in
      // the card view, which is what keeps the canvas a graph instead of
      // a spaghetti diagram.
      const coreEdges = allEdges
        .filter((edge) => edge.weight >= minEdgeWeight)
        .sort((a, b) => b.weight - a.weight)
        .slice(0, maxEdges)
      const coreSet = new Set()
      for (const edge of coreEdges) {
        coreSet.add(edge.source)
        coreSet.add(edge.target)
      }
      const coreNodes = [...nodes.values()]
        .filter((node) => coreSet.has(node.text))
        .sort((a, b) => b.degree - a.degree || b.count - a.count || b.lastSeen - a.lastSeen)
        .slice(0, maxNodes)
      return { cards, graph: { nodes: coreNodes, edges: coreEdges } }
    },
  ...
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/memory/api/purge',
    handler: async (req, res) => {
      if (req.method !== 'POST') return json(res, 405, { ok: false, error: 'POST only' })
      const body = await readBody(req)
      json(res, 200, await service.purge(String(body.id ?? '')))
    },
  }), 'dsh-memory: purge route')
  ...
  if (remoteEnabled) {
    ...
    register('/memory/remote/purge', async (req, res) => {
      if (req.method !== 'POST') return json(res, 405, { ok: false, error: 'POST only' })
      const body = await readBody(req)
      json(res, 200, await service.purge(String(body.id ?? '')))
    }, 'purge')
