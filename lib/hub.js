...
/** Retrieval score: importance × confidence × recency; unverified entries halve. */
function score(entry) {
  const unverified = entry.unverified === true ? 0.5 : 1
  return (entry.importance ?? 0.5) * (entry.confidence ?? 1) * decay(entry) * unverified
}
  ...
  const service = {
    ...
    async remember(entry, source = {}) {
      ...
      if (duplicate !== undefined) {
        ...
        duplicate.unverified = entry.unverified === true
