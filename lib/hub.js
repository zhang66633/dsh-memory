...
export function apply(ctx, config = {}) {
  ...
  const service = {
    ...
    async graphData({ maxNodes = 60, maxEdges = 90 } = {}) {
      ...
      // Connected entities first: what a graph is actually about. Isolated
      // nodes (single-occurrence prose tokens) fill remaining slots so the
      // card view can still list them.
