...
/**
 * Extraction instruction with the caller's candidate cap.
 * @param maxItems - hard cap the model is told about up front.
 */
export function extractionSystemPrompt(maxItems) {
  return `${EXTRACTION_SYSTEM_BASE}\nEmit at most ${maxItems} items.`
}
...
export async function completeText(llm, options) {
  ...
  const messages = [{
    ...
    source: { kind: 'plugin', plugin: '@zheexinn/dsh-memory' },
  }]
