...
export function entitiesOf(content, cjkTerms) {
  const entities = extractRuleEntities(content).map(({ text }) => text)
  const text = String(content ?? '')
  for (const term of cjkTerms) {
    if (text.includes(term) && !entities.includes(term)) entities.push(term)
  }
  return entities
}
...
export function buildGraph(entries) {
  ...
  for (const entry of entries) {
    ...
    entryEntities.set(entry.id, entities)
    const at = entry.last_accessed_at ?? entry.created_at ?? 0
    for (const entity of entities) {
      const node = nodes.get(entity) ?? { text: entity, count: 0, lastSeen: 0 }
      node.count += 1
