...
// Single quotes are apostrophes in English prose ("user's …") far more often
// than quote delimiters; only double-ish quotes open quoted entities.
const QUOTED = /["“”「」]([^"“”「」\n]{1,30})["“”「」]/g
// Unix-style paths must be ASCII segments (≥2, each with at least one
// alphanumeric — ".." is not a segment) so prose slashes and relative-link
// hints never masquerade as paths; drive-anchored Windows paths stay
// permissive but stop at Chinese/full-width punctuation.
const PATH_END = '\\s，。;；：:！？、\'"(){}（）【】《》'
const WHOLE = new RegExp(
  `(workspace:[^${PATH_END}]+|[a-zA-Z]:[\\\\/][^${PATH_END}]+|(?:\\/[A-Za-z0-9._-]*[A-Za-z0-9][A-Za-z0-9._-]*){2,}\\/?|https?:\\/\\/[^${PATH_END}]+|\\bv?\\d+\\.\\d+(?:\\.\\d+)?(?:[-+][\\w.-]+)?|[A-Za-z_][A-Za-z0-9_-]{2,})`,
  'g',
)
const CJK_RUN = /[\u4e00-\u9fff]+/g
...
export function entitiesOf(content, cjkTerms, identifierDf) {
  ...
  // strong names: ALL-CAPS acronyms (BOM) and all-lowercase compounds
  // (math_agent, dsh-memory) are signals even once. One-off lowercase prose
  // words and mixed-case compounds (New-Item, ItemType) are noise.
  const strong = (text) =>
    /^[A-Z0-9]{2,}$/.test(text)
    || (/[_-]/.test(text) && text === text.toLowerCase())
